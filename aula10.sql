...
CREATE VIEW v_depcidade 
AS SELECT 
d.nome AS nome_departamento,
c.nome AS nome_cidade
FROM tbl_departamentos d, tbl_cidades c
WHERE d.cod_cidade = c.cod_cidade;
...
CREATE VIEW v_depcidadehouston 
AS SELECT *
FROM v_depcidade
...
CREATE VIEW vw_empregados_salarial
AS SELECT nome AS nome, salario AS salario
FROM tbl_empregados 
WHERE salario > 5500;
...
CREATE VIEW v_departemp 
AS SELECT d.nome AS departamento,
COUNT(e.cod_empregado) AS quantidade
FROM tbl_departamentos d
INNER JOIN tbl_empregados e
...
LEFT JOIN (
  SELECT cod_departamento, COUNT(cod_empregado) AS quantidade_empregados
  FROM tbl_empregados
  GROUP BY cod_departamento
) e ON d.cod_departamento = e.cod_departamento
GROUP BY d.nome, e.quantidade_empregados
HAVING COUNT(DISTINCT p.cod_projeto) > COALESCE(e.quantidade_empregados, 0);
