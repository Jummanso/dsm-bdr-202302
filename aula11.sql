...
--recebe como parametro um codigo inteiro e um novo nome em texto atualizando o nome do departamento com esse respectivo codigo
    ...
    CREATE PROCEDURE proc_upd_nome_depart (
...
BEGIN
    ...
    CREATE TEMP TABLE tbl_cidades_copia AS
    SELECT *
    FROM tbl_cidades
    WITH NO DATA;  

    INSERT INTO tbl_cidades_copia
    SELECT *
    FROM tbl_cidades;
    END $$;
...
CREATE PROCEDURE proc_novoprojeto(
...
CREATE PROCEDURE proc_delprojeto(
...
--a procedure devera criar uma tabela chamada tbl_projetos_arquivados, caso ela nao exista. A tabela deve ter 2 colunas: codigo_projeto e nome.
--a procedure deve salvar o projeto do codigo recebido na tbl_projetos_arquivados e deleta-la da tabela tbl_projetos.

CREATE PROCEDURE proc_projeto_arquivado(
    IN p_cod_projeto INT
)
LANGUAGE plpgsql
AS $$
