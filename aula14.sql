...
SELECT cod_departamento, qtde_projetos FROM tbl_departamentos;

--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado

CREATE OR REPLACE FUNCTION atualizar_qtde_projetos_novo_projeto()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_atualizar_qtde_projetos_novo_projeto
AFTER INSERT ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION atualizar_qtde_projetos_novo_projeto();

Exemplo uso:

INSERT INTO tbl_projetos (nome, cod_departamento) VALUES ('Novo Projeto', 1);
...
--Associar o gatilho à tabela tbl_empregados
...
-- Associar a trigger à tabela tbl_empregados
